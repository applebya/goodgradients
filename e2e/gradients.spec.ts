...
test.describe("GoodGradients - Gradient Detail", () => {
  ...
  test("should open gradient detail modal when clicking a card", async ({
    page,
  }) => {
  ...
  test("should show collapsible gradient settings", async ({ page }) => {
    ...
    await expect(
      page.getByRole("dialog").getByRole("button", { name: /Linear/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("dialog").getByRole("button", { name: /Radial/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("dialog").getByRole("button", { name: /Conic/i }),
    ).toBeVisible();
  });
  ...
  test("should change gradient type in settings", async ({ page }) => {
    ...
    await page
      .getByRole("dialog")
      .getByRole("button", { name: /Radial/i })
      .click();

    // Radial button should now be active
    const radialButton = page
      .getByRole("dialog")
      .getByRole("button", { name: /Radial/i });
  ...
  test("should show code export tabs", async ({ page }) => {
    ...
    await expect(
      page.getByRole("dialog").locator("button").filter({ hasText: "CSS" }),
    ).toBeVisible();
  });
  ...
  test("should close fullscreen preview and not persist across modals", async ({
    page,
  }) => {
...
test.describe("GoodGradients - URL State", () => {
  ...
  test("should update gradient type when clicking buttons", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector('[data-testid="gradient-card"]', {
    ...
    const conicButton = page
      .getByRole("dialog")
      .getByRole("button", { name: /Conic/i });
...
test.describe("GoodGradients - Favorites", () => {
  test("should favorite gradient on first click with fresh localStorage", async ({
    page,
  }) => {
    ...
    // Reload page with cleared storage
    await page.reload();
    await page.waitForSelector('[data-testid="gradient-card"]', {
      timeout: 15000,
    });
    ...
    // Find the "Add to favorites" button (not yet favorited)
    const addButton = firstCard.locator(
      'button[aria-label*="Add"][aria-label*="favorites"]',
    );
    ...
    // Re-hover to ensure buttons are visible
    await firstCard.hover();

    // Verify heart changes to filled state (aria-label should now contain "Remove")
    const removeButton = firstCard.locator(
      'button[aria-label*="Remove"][aria-label*="favorites"]',
    );
...
test.describe("GoodGradients - Filter Bar", () => {
  ...
  test("should show filter dropdowns on desktop", async ({ page }) => {
    ...
    await expect(
      page.locator('button[aria-label="Filter by colors"]'),
    ).toBeVisible();

    // Tags/vibes dropdown should be visible
    await expect(
      page.locator('button[aria-label="Filter by tags"]'),
    ).toBeVisible();

    // Gradient type dropdown should be visible
    await expect(
      page.locator('button[aria-label="Gradient type"]'),
    ).toBeVisible();
  });
